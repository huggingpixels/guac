...
func validateS3Opts(graphqlEndpoint string, csubAddr string, csubTls bool, csubTlsSkipVerify bool, s3url string, s3bucket string, s3path string, region string, s3item string, mp string, mpEndpoint string, queues string, poll bool) (s3Options, error) {
	var opts s3Options
	...
	opts = s3Options{s3url, s3bucket, s3path, s3item, region, queues, mp, mpEndpoint, poll, graphqlEndpoint, csubClientOptions}
